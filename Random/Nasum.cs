...
{
	/// <summary>
	/// A random number generator class which uses a single internal seed to generate values. Unlike Unity's Random
	/// class, this class' generated values are guaranteed to be the same across invokations if both the seeds and the
	/// method invokation orders are the same. This class is not thread-safe.
	/// </summary>
	public static class Nasum
	{
		...
		/// <summary>
		/// Returns a random number generated using the standard distribution with the given <paramref name="mean"/> and
		/// <paramref name="standardDeviation"/>, in the range given by <paramref name="rangeMin"/> and
		/// <paramref name="rangeMax"/>. The algorithm first obtains a gaussian random variable in a standard way, and
		/// then checks if it's within the given range. If it's not, the random roll is repeated until a value in the
		/// [rangeMin, rangeMax] range is received. This can cause an infinite loop if you're not careful with
		/// parameters.
		/// </summary>
