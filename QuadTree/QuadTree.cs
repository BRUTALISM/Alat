...
{
	/// <summary>
	/// The generic quad tree class. Its elements need to have a 2D coordinate representation. If the given maximum
	/// number of elements per tree is exceeded, the tree automatically splits itself into four subtrees, and transfers
	/// all of its elements to them. Similarly, if an element is added that sits outside the quadtree's boundaries, a
	/// parent subtree is created and the given subtree is set as one of its four subtrees.
	/// Check out http://en.wikipedia.org/wiki/Quadtree for more info.
	/// </summary>
	public class QuadTree<T> where T : IPoint2
	{
		...
		/// <summary>
		/// The subtrees as a read-only collection. Needed for inspection, such as for drawing gizmos in the Unity
		/// editor.
		/// </summary>
		...
		/// <summary>
		/// Adds the specified element to the quad tree. If the element is outside of the quad tree's bounds, this
		/// method creates a new parent quad tree, sets the current tree as one of its subtrees, and returns the parent.
		...
		/// within a circle defined by the element's center and the given radius, but within the minimal square drawn
		/// around that circle.
		/// </param>
		public List<T> GetNeighbours(T element, float blockyRadius)
		{
			return GetNeighboursRecursive(new Vector2(element.Position.x, element.Position.y), blockyRadius, new List<T>());
		}
		
		/// <summary>
		/// Gets the neighbours surrounding the given position.
		/// </summary>
		/// <returns>
		/// The neighbours.
		/// </returns>
		...
		/// </param>
		/// <param name='blockyRadius'>
		/// The "radius" of the search around the given position. It's called blocky because the search is not performed
		/// within a circle defined by the position and the given radius, but within the minimal square drawn around
		/// that circle.
		/// </param>
		public List<T> GetNeighbours(Vector2 position, float blockyRadius)
		{
			return GetNeighboursRecursive(position, blockyRadius, new List<T>());
		}
		...
		{
			if (element.Position.x <= CenterX)
			{
				if (element.Position.y <= CenterY) subtrees[0].Add(element);
				else subtrees[2].Add(element);
			}
			else
			{
				if (element.Position.y <= CenterY) subtrees[1].Add(element);
		...
		private QuadTree<T> CreateParent(T element)
		{
			...
			//   |0 1
			//   +--->x
			if (element.Position.x < minimumX)
			{
			...
			if (element.Position.y < minimumY)
			{
		...
		private List<T> GetNeighboursRecursive(Vector2 position, float blockyRadius, List<T> neighboursSoFar)
		{
			float leftBound = position.x - blockyRadius;
			float rightBound = position.x + blockyRadius;
			float bottomBound = position.y - blockyRadius;
			float topBound = position.y + blockyRadius;
			...
			if (!notOverlapping)
			{
				...
				{
					subtree.GetNeighboursRecursive(position, blockyRadius, neighboursSoFar);
				}
			}
		...
		{
			return node.Position.x >= left && node.Position.x <= right &&
				node.Position.y >= bottom && node.Position.y <= top;
		}
