...
{
	/// <summary>
	/// An infinite color palette which can be polled for new colors ad infinitum. The color generation algorithm is
	/// controlled by the parameters supplied at creation time.
	/// </summary>
	public class InfinitePalette
	{
		#region Primary color parameters
		/// <summary>
		/// The method used to generate initial (primary) colors which are used as starting points for generating an
		/// infinite palette.
		/// </summary>
		...
		/// <summary>
		/// The affinity of the algorithm towards selecting light and/or dark colors (with high and low Lightness
		/// values, respectively).
		/// </summary>
		...
		public class Parameters
		{
			...
			/// <summary>
			/// The affinity of the generation algorithm to pick very light or very dark colors (or both) as primary
			/// colors.
			/// </summary>
			...
			/// <summary>
			/// The angle increment to be used by primary color methods which use the hue offset technique (Analogous,
			/// Complementary).
			/// </summary>
			public float AnalogousHueAngleIncrement = 10f;
			
			/// <summary>
			/// The mean value for the Gaussian distribution when generating a random lightness value with the
			/// LightnessAffinity.Dark affinity.
			/// </summary>
			public float LightnessDarkAffinityMean = 0.2f;
			
			/// <summary>
			/// The mean value for the Gaussian distribution when generating a random lightness value with the
			/// LightnessAffinity.Light affinity.
			/// </summary>
			...
			/// <summary>
			/// When the generation algorithm iterates over primary columns, this is the affinity it will use to choose
			/// the next column.
			/// </summary>
			public DerivationSourceAffinity DerivationSourceAffinity;
			
			/// <summary>
			/// How much the column affinity is taken into account when selecting columns, in the [0, 1] range. A zero
			/// value means that the affinity will be ignored (effectively reducing column selection to a random
			/// choice), while a value of 1 will always select the column with the highest preference, ignoring other
			/// columns. The Truth is somewhere between 0 and 1.
			/// </summary>
			[Range(0f, 1f)]
			...
			// The three values below control the magnitude of the offset vector (in cylindrical space) from the current
			// color to the color about to be generated next.
		...
		public HSLColor BaseColor
		{
			get
			{
				...
				{
					throw new System.InvalidOperationException("base color not set, no colors have been generated");
				}
				return primaries[0];
			}
		}
		...
		public InfinitePalette(Parameters parameters)
		{
			if (parameters == null)
			{
				throw new System.ArgumentException("parameters are null, can't generate palette without them");
			}
		...
		private HSLColor GeneratePrimaryColor()
		{
			...
			switch (parameters.PrimaryColorMethod)
			{
				...
				case PrimaryColorMethod.Compound:
					var compoundOffset = primaries.Count > 0 ? compoundColorOffsets[(primaries.Count - 1) %
						compoundColorOffsets.Count] : 0f;
			...
			switch (parameters.LightnessAffinity)
			{
				...
				case LightnessAffinity.Dark:
					newColor.L = Nasum.GaussianInRange(parameters.LightnessDarkAffinityMean,
						parameters.LightnessAffinityStandardDeviation, LightnessBlack, LightnessMaximumColor);
					break;
				case LightnessAffinity.Light:
					newColor.L = Nasum.GaussianInRange(parameters.LightnessLightAffinityMean,
						parameters.LightnessAffinityStandardDeviation, LightnessMaximumColor, LightnessWhite);
					break;
				case LightnessAffinity.LightAndDark:
					newColor.L = Nasum.GaussianInRange(LightnessMaximumColor,
						parameters.LightnessAffinityStandardDeviation, LightnessBlack, LightnessWhite);
					break;
			}
		...
		private int PickWithAffinity(IEnumerable<HSLColor> orderedColors)
		{
			...
			var randomIndex = Mathf.RoundToInt(Nasum.GaussianInRange(0f, deviation, 0f, 1f) *
				(orderedColors.Count() - 1));
